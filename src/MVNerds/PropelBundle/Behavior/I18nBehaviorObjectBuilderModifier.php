...
/**
 * TWEAK par Eric Chau 
 * Dans le fichier PHP I18nBehavior qui se trouve dans le vendor propel, au lieu de la ligne :
 * require_once dirname(__FILE__) . '/I18nBehaviorObjectBuilderModifier.php';
 * il faut la remplacer par :
 * use MVNerds\PropelBundle\Behavior\I18nBehaviorObjectBuilderModifier;
 */
...
class I18nBehaviorObjectBuilderModifier
{
    ...
    public function objectMethods($builder)
    {
		...
		$this->modifyIsModifiedMethod($script);
	...
	protected function modifyIsModifiedMethod(&$script)
    {
        $script .= "
/**
 * Modification par Eric Chau pour surcharger la méthode isModified() de BaseObject pour qu'il sauvegarde tous les objets I18n modifié
 */
public function isModified()
{
	foreach (\$this->coll" . $this->behavior->getTable()->getPhpName() . "I18ns as \$i18nChild) {
		if (\$i18nChild->isModified()) {
			\$i18nChild->save();
		}
	}
